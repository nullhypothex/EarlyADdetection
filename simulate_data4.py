...
def simulate_users(start_uid, num_users=1, num_days=100, baseline_vectors=None):
    global USE_GROQ_API
    path = "Nsimulated_AD_users_groq.csv"
    ...
    for uid in tqdm(range(start_uid, start_uid + num_users), desc=f"Users {start_uid}-{start_uid + num_users - 1}"):
        ...
        last_label = None  # Track previous day's label for progression logic
        ...
        for day in range(last_completed_day + 1, num_days + 1):
            ...
            for video_title in video_titles:
                raw_label = determine_label_for_day(progression_type, day)

                # Enforce only forward progression Healthy -> MCI -> EarlyAD, no backwards
                if last_label is None:
                    label = raw_label
                elif last_label == "Healthy":
                    label = raw_label if raw_label in ["Healthy", "MCI"] else "MCI"
                elif last_label == "MCI":
                    label = raw_label if raw_label in ["MCI", "EarlyAD"] else "EarlyAD"
                else:
                    label = "EarlyAD"  # EarlyAD cannot revert back
...
if __name__ == "__main__":
    print("Resuming simulation for users 1–100 over 100 days...")
    ...
    groq_users_all = set(range(1, 101, 4))  # Every 4th user starting from 1
